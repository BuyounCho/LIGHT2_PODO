...
void RBFTSensor::RBFT_AddCANMailBox(){
    ...
    canHandler->RBCAN_AddMailBox(ID_RCV_MXMYFZ);
    canHandler->RBCAN_AddMailBox(ID_RCV_FXFYMZ);
}
...
int RBFTSensor::RBFT_RequestONOFF(bool ONOFF){
    ...
    mb.data[2] = ONOFF;         // 1 : Enable, 0 : Disable
    mb.dlc = 3;
...
//            ROLL = AccRoll/SFRoll;
//            VelRoll = dAccRoll;//(double)((1.0f - 2.0f*RBCORE_PI*CutOffFeq*(double)RT_TIMER_PERIOD_MS/1000.0f)*VelRollOld + (double)RT_TIMER_PERIOD_MS/1000.0f*AccRollOld);
//            VelRollOld = VelRoll;
//            VelPitch = dAccPitch;//(double)((1.0f - 2.0f*RBCORE_PI*CutOffFeq*(double)RT_TIMER_PERIOD_MS/1000.0f)*VelPitchOld + (double)RT_TIMER_PERIOD_MS/1000.0f*AccPitchOld);
