...
a Python wrapper for a fast solver generated by FORCESPRO v4.2.0
   ...
   OUTPUT = Solver_PumpPressureControl_py.Solver_PumpPressureControl_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['xinit'] - column vector of length 2
       PARAMS['x0'] - column vector of length 48
       PARAMS['all_parameters'] - column vector of length 16
   ...
   OUTPUT returns the values of the last iteration of the solver where
       ...
       OUTPUT['x12'] - column vector of size 3
       OUTPUT['x13'] - column vector of size 3
       OUTPUT['x14'] - column vector of size 3
       OUTPUT['x15'] - column vector of size 3
       OUTPUT['x16'] - column vector of size 3

   [OUTPUT, EXITFLAG] = Solver_PumpPressureControl_py.Solver_PumpPressureControl_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = Solver_PumpPressureControl_py.Solver_PumpPressureControl_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'Solver_PumpPressureControl/lib/Solver_PumpPressureControl.dll')) 
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'Solver_PumpPressureControl/lib/Solver_PumpPressureControl.dll'))
    csolver = getattr(_lib,'Solver_PumpPressureControl_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'Solver_PumpPressureControl/lib/libSolver_PumpPressureControl.dll'))
    csolver = getattr(_lib,'Solver_PumpPressureControl_solve')

class Solver_PumpPressureControl_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('xinit', ctypes.c_double * 2),
('x0', ctypes.c_double * 48),
('all_parameters', ctypes.c_double * 16),
('reinitialize', ctypes.c_int),
]

Solver_PumpPressureControl_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
Solver_PumpPressureControl_params_types = {'xinit' : np.float64,
'x0' : np.float64,
'all_parameters' : np.float64,
'reinitialize' : np.int32,
}

class Solver_PumpPressureControl_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
('x12', ctypes.c_double * 3),
('x13', ctypes.c_double * 3),
('x14', ctypes.c_double * 3),
('x15', ctypes.c_double * 3),
('x16', ctypes.c_double * 3),
]

Solver_PumpPressureControl_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),
...
class Solver_PumpPressureControl_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
csolver.argtypes = ( ctypes.POINTER(Solver_PumpPressureControl_params_ctypes), ctypes.POINTER(Solver_PumpPressureControl_outputs_ctypes), ctypes.POINTER(Solver_PumpPressureControl_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def Solver_PumpPressureControl_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v4.2.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = Solver_PumpPressureControl_params_ctypes()
    for par in params_arg:
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=Solver_PumpPressureControl_params_types[par], requirements='F')
    ...
    outputs_py = Solver_PumpPressureControl_outputs_ctypes()
    info_py = Solver_PumpPressureControl_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.Solver_PumpPressureControl_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp)  )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.Solver_PumpPressureControl_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), None )
            else:
                exitflag = _lib.Solver_PumpPressureControl_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), fp )
            libc.fclose(fp)
    ...
    # convert outputs
    for out in Solver_PumpPressureControl_outputs:
        Solver_PumpPressureControl_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return Solver_PumpPressureControl_outputs,int(exitflag),info_py

solve = Solver_PumpPressureControl_solve
