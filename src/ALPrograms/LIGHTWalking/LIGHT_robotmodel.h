...
// LIGHT's Wholebody desired state ///////////////////////////////////////////////////////
//
struct LIGHT_WB_RDES
{
    // for kinematics
    MatrixNd Qdes;      // Desired Output Ankle
    MatrixNd dQdes;     // Desired Output Angular Vel.

    // for dynamics
    MatrixNd ddQdes;    // Desired Output Angular Acc.
    MatrixNd Tdes;      // Desired Output Torque
    MatrixNd Fext_des;      // Desired Output Force to Environment

    LIGHT_WB_RDES(){
        Qdes = MatrixNd::Zero(LIGHT_DOF,1);
        dQdes = MatrixNd::Zero(LIGHT_DOF,1);
        ddQdes = MatrixNd::Zero(LIGHT_DOF,1);

        Tdes = MatrixNd::Zero(LIGHT_ACT_DOF,1);
        Fext_des = MatrixNd::Zero(LIGHT_NUM_CON,1);
    }
};

// LIGHT's Wholebody reference variable(actuator level) ///////////////////////////////////////////////////////
// Rotary : ankle, angular velocity
// Cylinder : linear position, velocity
struct LIGHT_FINAL_REF
{
    MatrixNd S_ref;      // Actuator Level Position Reference
    MatrixNd dS_ref;     // Actuator Level Velocity Reference
    MatrixNd F_ref;      // Actuator Level Force/Torque Reference

    LIGHT_FINAL_REF(){
        S_ref = MatrixNd::Zero(LIGHT_ACT_DOF,1);
        dS_ref = MatrixNd::Zero(LIGHT_ACT_DOF,1);
        F_ref = MatrixNd::Zero(LIGHT_ACT_DOF,1);
    }
};
...
public:
    ...
    LIGHTWholeBody()
    {
        ...
        ddQref_Comp = VectorNd::Zero(LIGHT_DOF);
        ...
        K_RF_pos_OnContact(0) = 50.0; // N/m
        K_RF_pos_OnContact(1) = 50.0;
        K_RF_pos_OnContact(2) = 50.0;
        D_RF_ori_OnContact(0) = 3.0;
        D_RF_ori_OnContact(1) = 2.0;
        D_RF_ori_OnContact(2) = 2.0;
        D_RF_pos_OnContact(0) = 5.0;
        D_RF_pos_OnContact(1) = 5.0;
        D_RF_pos_OnContact(2) = 5.0;

        K_RF_ori_OffContact(0) = 100.0; // Nm/rad
...
//        K_LF_ori_OnContact(0) = 100.0; // Nm/rad
//        K_LF_ori_OnContact(1) = 100.0;
//        K_LF_ori_OnContact(2) = 100.0;
//        K_LF_pos_OnContact(0) = 50.0; // N/m
//        K_LF_pos_OnContact(1) = 50.0;
//        K_LF_pos_OnContact(2) = 50.0;
//        D_LF_ori_OnContact(0) = 3.0;
//        D_LF_ori_OnContact(1) = 2.0;
//        D_LF_ori_OnContact(2) = 2.0;
//        D_LF_pos_OnContact(0) = 5.0;
//        D_LF_pos_OnContact(1) = 5.0;
//        D_LF_pos_OnContact(2) = 5.0;

//        K_LF_ori_OffContact(0) = 100.0; // Nm/rad
//        K_LF_ori_OffContact(1) = 50.0;
//        K_LF_ori_OffContact(2) = 50.0;
//        K_LF_pos_OffContact(0) = 300.0; // N/m
//        K_LF_pos_OffContact(1) = 300.0;
//        K_LF_pos_OffContact(2) = 300.0;
//        D_LF_ori_OffContact(0) = 10.0;
//        D_LF_ori_OffContact(1) = 10.0;
//        D_LF_ori_OffContact(2) = 10.0;
//        D_LF_pos_OffContact(0) = 10.0;
//        D_LF_pos_OffContact(1) = 10.0;
//        D_LF_pos_OffContact(2) = 10.0;
