...
int main(int argc, char *argv[])
{
    ...
    while(__IS_WORKING){
        ...
        case SH_TASK_SINEWAVE_OPENING:
        {
            ...
            _BNO = sharedCMD->COMMAND[PODO_NO].USER_PARA_INT[0]; // Target board
            FILE_LOG(logWARNING) << "Target Board Number : " << _BNO;
            ...
            if(_BNO == 4||_BNO == 5) {
                ...
                FILE_LOG(logWARNING) << "Target Board Number : " << _BNO ;
                ValveOpenmodeON(_BNO,0);
            }

            double SINE_TIME = sharedCMD->COMMAND[PODO_NO].USER_PARA_DOUBLE[0]/1000.0; // pulse
            double SINE_MAG = sharedCMD->COMMAND[PODO_NO].USER_PARA_DOUBLE[1]; // pulse
            double SINE_NUM = sharedCMD->COMMAND[PODO_NO].USER_PARA_DOUBLE[2]; // pulse

            double SINE_DT = 0.010; // 10msec
            double SINE_IDX = 0.0;
            save_Flag = true;
            while(1){
                double OPENING_d = SINE_MAG*sin(2.0*3.1415*SINE_IDX*SINE_DT/SINE_TIME);
                int OPENING = (int)OPENING_d;
                ...
                if(_BNO == 4||_BNO == 5) {
                    ...
                    jCon->SetRefValvePos(_BNO,OPENING);
                }
                usleep(SINE_DT*1000.0*1000.0);
                if (SINE_IDX >= SINE_NUM*SINE_TIME/SINE_DT) {
                    break;
                } SINE_IDX=SINE_IDX+1.0;
            }
            save_Flag = false;
            save_File();

            FILE_LOG(logSUCCESS) << "Sinewave Opening is Done!";
        ...
        case SH_TASK_TORQUE_CONTROL_GO:
        {
            ...
            _BNO = sharedCMD->COMMAND[PODO_NO].USER_PARA_INT[0]; // Board Number
            double REF_TORQUE = sharedCMD->COMMAND[PODO_NO].USER_PARA_DOUBLE[0]; // [Nm]

            TorCtrlmodeON(_BNO);
            LIGHTJoints.Tref(_BNO) = REF_TORQUE;
            FILE_LOG(logSUCCESS) << "Torque Control Start!";
            FILE_LOG(logSUCCESS) << "Output Torque : " << REF_TORQUE;
...
void PosCtrlmodeON(int BNO)
{
//    double CurrentRefAngle = sharedSEN->ENCODER[BNO][0].CurrentRefAngle;
//    jCon->SetJointRefAngle(BNO,CurrentRefAngle);
...
bool LIGHTJointSet::PosCtrl_GOTO_Pose() {
    ...
    if(_OPERATION_STAGE == 0) {
        ...
        double Ref_Pos = (CurrentAngle-DesiredAngle)*(1.0+cos(3.1415*IDX*SYS_DT/MOVING_TIME))/2.0 + DesiredAngle;
        double Ref_Vel = -(CurrentAngle-DesiredAngle)*(3.1415/MOVING_TIME)*sin(3.1415*IDX*SYS_DT/MOVING_TIME)/2.0;
        Qref(JNO) = Ref_Pos*D2R;
        dQref(JNO) = Ref_Vel*D2R;
        if (IDX >= MOVING_TIME*SYS_FREQ) {
            dQref(JNO) = 0.0;
            IDX = 0.0;
            _OPERATION_STAGE++;
        } IDX=IDX+1.0;
...
bool LIGHTJointSet::PosCtrl_Random_Motion() {
    ...
    if(_OPERATION_STAGE == 0) {
        ...
        for(int idx = 1; idx<7; idx++) {
            double k = (double)idx;
            Ref_Pos += a[idx]*cos(2.0*3.1415*k*SYS_DT*IDX);
            Ref_Vel += -a[idx]*2.0*3.1415*k*sin(2*3.1415*k*SYS_DT*IDX);
        }
        Qref(JNO) = Ref_Pos*D2R;
        dQref(JNO) = Ref_Vel*D2R;
...
bool LIGHTJointSet::PosCtrl_Sine_Motion() {
    ...
    if(_OPERATION_STAGE == 0) { // Parameters Setting
        ...
        for(int i=0;i<=MAX_PREVIEW;i++) {
            double t_view = t + SYS_DT + dT_window*(double)(i);
            if(t_view < t_init) {
...
void save_PutData(unsigned int cur_Index)
{
    ...
    save_Buf[8][cur_Index] = sharedSEN->ENCODER[RHY][0].CurrentAngVel;
...
//    save_Buf[19][cur_Index] = sharedSEN->ENCODER[RHP][0].CurrentRefValve;
...
//    save_Buf[29][cur_Index] = sharedSEN->ENCODER[RKN][0].CurrentRefValve;
...
//    save_Buf[39][cur_Index] = sharedSEN->ENCODER[RAP][0].CurrentRefValve;
...
//    save_Buf[49][cur_Index] = sharedSEN->ENCODER[RAR][0].CurrentRefValve;
//    save_Buf[50][cur_Index] = sharedSEN->ENCODER[RAR][0].CurrentValvePos;
    ...
    save_Buf[53][cur_Index] = sharedSEN->ENCODER[LHR][0].CurrentAngVel;
    ...
    save_Buf[58][cur_Index] = sharedSEN->ENCODER[LHY][0].CurrentAngVel;
...
//    save_Buf[69][cur_Index] = sharedSEN->ENCODER[LHP][0].CurrentRefValve;
...
//    save_Buf[79][cur_Index] = sharedSEN->ENCODER[LKN][0].CurrentRefValve;
...
//    save_Buf[89][cur_Index] = sharedSEN->ENCODER[LAP][0].CurrentRefValve;
...
//    save_Buf[99][cur_Index] = sharedSEN->ENCODER[LAR][0].CurrentRefValve;
...
void save_File(char *filecomment)
{
    ...
    std::stringstream ss;
    ss << std::put_time(std::localtime(&in_time_t), "SH_AL_Data/%Y%m%d_%X");
