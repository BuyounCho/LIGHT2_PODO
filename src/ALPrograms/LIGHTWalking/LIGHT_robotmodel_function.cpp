...
//    dQnew(PELVIS_ORI_QNUMSTART+2) = sharedSEN->IMU[0].Wz_G;
    ...
    // LowPass Filtering
    double f_cut_pelori = 20.0; // not good under 10.0!
    ...
    if(Flag_PelvisPositionEstimation) {
        if(Cur_RefFrame == REFFRAME_PEL) {
            ...
            double K = 2.0*3.1415*50.0*SYS_DT;
            Vector3d Vnow_Pel_Update = Vnow_Pel_Old + K*(Vnow_Pel_Kin - Vnow_Pel_Old) + Anow_Pel_Acc*SYS_DT;
            dQnew.segment(PELVIS_POS_QNUMSTART,3) = Vnow_Pel_Update;

            K = 2.0*3.1415*50.0*SYS_DT;
            Vector3d Xnow_Pel_Update = Xnow_Pel_Old + K*(Xnow_Pel_Kin - Xnow_Pel_Old) + Vnow_Pel_Update*SYS_DT;
...
//                                                           f_cut_pelpos);
    ...
    double f_cut_JTS = 20.0; // not good under 10.0!
    ...
    double f_cut_EXT = 20.0; // not good under 10.0!
...
void LIGHTWholeBody::CheckStates() {
    ...
    if (Flag_StateCheck) {
        ...
        if(fabs(Angle_Roll)>30.0*D2R) {
            Flag_UnnormalStates = true; return;
        }

        if(fabs(Angle_Pitch)>30.0*D2R) {
            Flag_UnnormalStates = true; return;
        }
...
void LIGHTWholeBody::SetReference_Joint(bool UpdateEnable, bool PelvisCompensation)
{
    if(UpdateEnable) {
        ...
        for(int idx=0; idx<LIGHT_ACT_DOF; idx++){
            _REF_ANGVEL = dQref(FLOATING_BASE_DOF+idx, 0);
        ...
        for(int idx=0; idx<LIGHT_ACT_DOF; idx++){
            double f_cut = 20.0; // Ankle torque reference
            Tref_new(idx) = Tref(idx, 0);
            Tref_fil(idx) = LPF_2nd(Tref_oold(idx),Tref_old(idx),Tref_new(idx),f_cut);
//                Tref_fil(idx) = LPF_1st(Tref_old(idx),Tref_new(idx),f_cut);
        }
        Tref_oold = Tref_old;
        Tref_old = Tref_fil;
...
void LIGHTWholeBody::SetReference_Actuator(bool UpdateEnable)
{
    ...
    if(UpdateEnable) {
        ...
        _Tref = Tref(_JOINT); // joint torque
        ...
        _T1ref = Tref(_JOINT1); // Pitch joint torque
        _T2ref = Tref(_JOINT2); // Roll joint torque
...
void LIGHTWholeBody::SetActuatorStiffnDamp(bool UpdateEnable)
{
    ...
    for(int i=0;i<6;i++) {
        K_joint_RF_new(i) = K_joint_RF(i,i);
    }
    ...
    for(int i=0;i<6;i++) {
        D_joint_RF_new(i) = D_joint_RF(i,i);
    }
    ...
    for(int i=0;i<6;i++) {
        K_joint_LF_new(i) = K_joint_LF(i,i);
    }
    ...
    for(int i=0;i<6;i++) {
        D_joint_LF_new(i) = D_joint_LF(i,i);
    }
    ...
    K_act_RF(0) = K_joint_RF(0,0)/R2D; // N/rad >> N/deg
    K_act_RF(1) = K_joint_RF(1,1)/R2D; // N/rad >> N/deg
    K_act_LF(0) = K_joint_LF(0,0)/R2D; // N/rad >> N/deg
    K_act_LF(1) = K_joint_LF(1,1)/R2D; // N/rad >> N/deg
    D_act_RF(0) = D_joint_RF(0,0)/R2D; // N/(rad/s) >> N/(deg/s)
    D_act_RF(1) = D_joint_RF(1,1)/R2D; // N/(rad/s) >> N/(deg/s)
    D_act_LF(0) = D_joint_LF(0,0)/R2D; // N/(rad/s) >> N/(deg/s)
    D_act_LF(1) = D_joint_LF(1,1)/R2D; // N/(rad/s) >> N/(deg/s)
    ...
    K_act_RF(2) = K_joint_RF(2,2)/J_temp/J_temp*0.001; // N/m >> N/mm
    D_act_RF(2) = D_joint_RF(2,2)/J_temp/J_temp*0.001; // N/(m/s) >> N/(mm/s)
    J_temp = AngularVel2CylinderVel_Hip(Qnow(FLOATING_BASE_DOF+LHP), 1.0); // joint error(rad) >> cylinder error(mm)
    K_act_LF(2) = K_joint_LF(2,2)/J_temp/J_temp*0.001; // N/m >> N/mm
    D_act_LF(2) = D_joint_LF(2,2)/J_temp/J_temp*0.001; // N/(m/s) >> N/(mm/s)
    ...
    K_act_RF(3) = K_joint_RF(3,3)/J_temp/J_temp*0.001; // N/m >> N/mm
    D_act_RF(3) = D_joint_RF(3,3)/J_temp/J_temp*0.001; // N/(m/s) >> N/(mm/s)
    J_temp = AngularVel2CylinderVel_Knee(Qnow(FLOATING_BASE_DOF+LKN), 1.0); // joint error(rad) >> cylinder error(mm)
    K_act_LF(3) = K_joint_LF(3,3)/J_temp/J_temp*0.001; // N/m >> N/mm
    D_act_LF(3) = D_joint_LF(3,3)/J_temp/J_temp*0.001; // N/(m/s) >> N/(mm/s)
    ...
    M2_temp = J2_temp.transpose().inverse()*K_joint_RF.block(4,4,2,2)*J2_temp.inverse();
    K_act_RF(4) = M2_temp(0,0)*0.001;  // N/m >> N/mm
    K_act_RF(5) = M2_temp(1,1)*0.001;  // N/m >> N/mm
    M2_temp = J2_temp.transpose().inverse()*D_joint_RF.block(4,4,2,2)*J2_temp.inverse();
    ...
    M2_temp = J2_temp.transpose().inverse()*K_joint_LF.block(4,4,2,2)*J2_temp.inverse();
    K_act_LF(4) = M2_temp(0,0)*0.001;  // N/m >> N/mm
    K_act_LF(5) = M2_temp(1,1)*0.001;  // N/m >> N/mm
    M2_temp = J2_temp.transpose().inverse()*D_joint_LF.block(4,4,2,2)*J2_temp.inverse();
