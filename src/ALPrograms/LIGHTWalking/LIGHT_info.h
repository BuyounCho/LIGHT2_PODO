...
public :
    ...
    double limit_inside, limit_outside, limit_toe, limit_heel;

    // Step Constraints
    double YawStepInside_Saturation, YStepInside_Saturation;
    ...
    INFO_LIGHT()  //set initial value.
    {
        ...
        m_torso = 28.0; // (with HPU) // 17.218 (without HPU)
        m_pel = 4.498;
...
//        I_lar = Matrix3d(1.560, -0.012, 0.639, -0.012, 1.750, -0.0075, 0.639, -0.0075, 1.377)*0.001*30.0;
        ...
        I_rap = Matrix3d(0.035, 0.0, 0.0, 0.0, 0.0535, 0.0, 0.0, 0.0, 0.0437)*0.001*1.0;
        I_rar = Matrix3d(1.560, 0.0, 0.639, 0.0, 1.750, 0.0, 0.639, 0.0, 1.377)*0.001*30.0;

        I_lhr = Matrix3d(0.463, 0.0, 0.0, 0.0, 1.677, 0.0, 0.0, 0.0, 1.665)*0.001*1.0;
        I_lhy = Matrix3d(0.789, 0.0, 0.0, 0.0, 0.496, 0.0, 0.0, 0.0, 0.433)*0.001*1.0;
        I_lhp = Matrix3d(30.69, 0.0, 14.13, 0.0, 111.743, 0.0, 14.13, 0.0, 94.59)*0.001*1.0;
        I_lkn = Matrix3d(34.27, 0.0, 13.06, 0.0, 40.03, 0.0, 13.06, 0.0, 11.38)*0.001*1.0;
        I_lap = Matrix3d(0.035, 0.0, 0.0, 0.0, 0.0535, 0.0, 0.0, 0.0, 0.0437)*0.001*1.0;
        I_lar = Matrix3d(1.560, 0.0, 0.639, 0.0, 1.750, 0.0, 0.639, 0.0, 1.377)*0.001*30.0;


        //center of mass_lb (absolute frame) / Unit : [m]
        c_torso = Vector3d(-30.0, 0.0, 250.00) * MM2M; // = Vector3d(8.25, 0.0, 318.68) * MM2M;
        ...
        footsize_inside = 0.075;
        footsize_outside = 0.075;
        footsize_toe = 0.140;
        footsize_heel = 0.140;
        limit_inside = footsize_inside;
        limit_outside = footsize_outside;
        limit_toe = footsize_toe;
        limit_heel = footsize_heel;
//        limit_inside = footsize_inside - 0.01;
//        limit_outside = footsize_outside - 0.01;
//        limit_toe = footsize_toe - 0.020;
//        limit_heel = footsize_heel - 0.020;

        YawStepInside_Saturation = 8.0*D2R;
        YStepInside_Saturation = footsize_inside*2.0 + 0.02;
