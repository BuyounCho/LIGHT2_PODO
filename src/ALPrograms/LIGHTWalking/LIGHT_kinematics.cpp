...
//        V_des(0) = dXdes_RF2CoM(0) + wn*(Xdes_RF2CoM(0)-(-Xref_Pel2RF(0)));
//        V_des(1) = dXdes_RF2CoM(1) + wn*(Xdes_RF2CoM(1)-(-Xref_Pel2RF(1)));
//        V_des(2) = dXdes_RF2CoM(2) + wn*(Xdes_RF2CoM(2)-(-Xref_Pel2RF(2)));
//        temp_A.block(0,0,1,n_var) = W*Jv_RF2Pel.row(0);
//        temp_A.block(1,0,1,n_var) = W*Jv_RF2Pel.row(1);
//        temp_A.block(2,0,1,n_var) = W*Jv_RF2Pel.row(2);
        temp_B.block(0,0,3,1) = W*V_des;
...
//    // 18) Velocity Change Minimization (6)
//    if(INFO_InvKin.Weight[INFO_InvKin.W_Change_Minimize] != 0.0) {
//        double W = INFO_InvKin.Weight[INFO_InvKin.W_Change_Minimize];
//        temp_size_cost = LIGHT_DOF;
//        Resize_TempMatrix(temp_size_cost,n_var,temp_A,temp_B);

//        temp_A.block(0,0,LIGHT_DOF,LIGHT_DOF) = MatrixNd::Identity(LIGHT_DOF,LIGHT_DOF);
//        temp_B.block(0,0,LIGHT_DOF,1) = dQref;// + ddQref*SYS_DT;

//        Matrix4QP_Addition(A_cost,B_cost,W*temp_A,W*temp_B);
//        idx_cost += temp_size_cost;
//    }
...
//        }
    ...
    if(Flag_KinematicCompensation) {
        double K_acc = 0.0;
        dQref_Comp(FLOATING_BASE_DOF+RHR) = dQref_Comp(FLOATING_BASE_DOF+RHR) + K_acc*ddQref(FLOATING_BASE_DOF+RHR)*SYS_DT;
        dQref_Comp(FLOATING_BASE_DOF+LHR) = dQref_Comp(FLOATING_BASE_DOF+LHR) + K_acc*ddQref(FLOATING_BASE_DOF+LHR)*SYS_DT;
    ...
    Qref.segment(0,3) = Qref.segment(0,3) + dQref.segment(0,3)*SYS_DT;
    ...
    Quat_Pelvis += 0.5*SYS_DT*dQuat_Pelvis;
    ...
    Qref.segment(ACT_QNUMSTART,LIGHT_ACT_DOF) = Qref.segment(ACT_QNUMSTART,LIGHT_ACT_DOF)
            + dQref.segment(ACT_QNUMSTART,LIGHT_ACT_DOF)*SYS_DT;
