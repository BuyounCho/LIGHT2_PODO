...
void PumpFlowControl_Active_LIGHT2(double Ps_now, double Ps_ref, double dPs_ref, double &u_out)
{
    ...
    Area_A[WST] = 9595.0*1e-9; //rotary, rA = m^3
    ...
    Area_B[WST] = 9595.0*1e-9; //rotary, rA = m^3
    ...
    JointVel[RHP] = sharedSEN->ENCODER[RHP][0].CurrentActVel * 0.001; // m/s
    JointVel[RKN] = sharedSEN->ENCODER[RKN][0].CurrentActVel * 0.001; // m/s
    JointVel[RAP] = sharedSEN->ENCODER[RAP][0].CurrentActVel * 0.001; // m/s
    JointVel[RAR] = sharedSEN->ENCODER[RAR][0].CurrentActVel * 0.001; // m/s
    JointVel[LHR] = sharedSEN->ENCODER[LHR][0].CurrentActVel * 3.1415/180; // rad/s
    JointVel[LHY] = sharedSEN->ENCODER[LHY][0].CurrentActVel * 3.1415/180; // rad/s
    JointVel[LHP] = sharedSEN->ENCODER[LHP][0].CurrentActVel * 0.001; // m/s
    JointVel[LKN] = sharedSEN->ENCODER[LKN][0].CurrentActVel * 0.001; // m/s
    JointVel[LAP] = sharedSEN->ENCODER[LAP][0].CurrentActVel * 0.001; // m/s
    JointVel[LAR] = sharedSEN->ENCODER[LAR][0].CurrentActVel * 0.001; // m/s
    JointVel[WST] = 0.0 * 3.1415/180; // rad/s

    Qflow_ref_JointVel = 0.0;
    static double JointVel_LPF[12] = {0.0,};
    for(int i=0;i<NO_OF_JOINTS;i++) {
        double alpha_update_vel = 1.0/(1.0+SYS_FREQ/(2.0*3.1415*20.0));
    ...
    double n_act = 13.0;
    Qflow_ref_Leakage = n_act*K_leak*(Ps_ref); // [LPM], Moog Valve : 12EA
    ...
    P_error_sum = P_error_sum + P_error*SYS_DT;

    double Kp = 0.15, Ki = 0.50;
    if(Ps_des[0] > P_ModeChange) {
        double dP_star = dPs_ref + (2.0*3.1415*3.0)*P_error;
    ...
    double alpha_PumpSpeed = 1.0/(1.0+SYS_FREQ/(2.0*3.141592*4.0));
    ...
    if(fabs(Ki)>1e-5) {
        ...
        if(wp_ref_LPF > wp_ref_max) {
            P_error_sum = P_error_sum - Ka*(wp_ref_LPF-wp_ref_max)*OutputFlowPerRev*SYS_DT;
            wp_ref_LPF = wp_ref_max;
        } else if(wp_ref_LPF < wp_ref_min) {
            P_error_sum = P_error_sum - Ka*(wp_ref_LPF-wp_ref_min)*OutputFlowPerRev*SYS_DT;
            wp_ref_LPF = wp_ref_min;
        }
        double alpha_decay = 1.0/(1.0+SYS_FREQ/(2.0*3.141592*0.3));
...
//        Ki = 0.10;
    ...
    double alpha_PumpSpeed = 1.0/(1.0+SYS_FREQ/(2.0*3.141592*30.0));
    wp_ref_LPF=(1.0-alpha_PumpSpeed)*wp_ref_LPF+alpha_PumpSpeed*wp_des;
